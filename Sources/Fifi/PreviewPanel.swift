    ...
    var body: some View {
        Group {
            if let item {
                ...
                EmptyStateView(systemImage: "sidebar.right", title: "No selection")
            }
        }
    ...
    @ViewBuilder private func actions(for item: ClipboardItem) -> some View {
        VStack(spacing: LeafiyDesign.Spacing.xs) {
            Button {
                ...
                Label("Send to Clipboard", systemImage: "doc.on.clipboard").frame(maxWidth: .infinity)
            }
            switch item.type {
            case .url:
                Button { viewModel.performQuickAction(.openURL, item: item) } label: {
                    Label("Open URL", systemImage: "safari").frame(maxWidth: .infinity)
                }
                Button { viewModel.performQuickAction(.copyCleanURL, item: item) } label: {
                    Label("Copy Without Tracking", systemImage: "wand.and.stars").frame(maxWidth: .infinity)
                }
            case .color:
                HStack {
            ...
            default:
                Button { viewModel.performQuickAction(.copyPlainText, item: item) } label: {
                    Label("Copy as Plain Text", systemImage: "text.alignleft").frame(maxWidth: .infinity)
                }
            }
        }
    }
