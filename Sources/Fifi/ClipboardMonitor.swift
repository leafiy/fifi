...
@MainActor final class ClipboardMonitor {
    ...
    init(historyStore: HistoryStore, blobStore: BlobStore, ignoreRulesStore: IgnoreRulesStore, settingsStore: SettingsStore) {
        ...
        selfWriteObserver = NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in
                guard let self,
                      let changeCount = notification.userInfo?["changeCount"] as? Int else { return }
                self.selfWriteChangeCounts.insert(changeCount)
                if self.selfWriteChangeCounts.count > 16 {
                    self.selfWriteChangeCounts.removeAll(keepingCapacity: true)
                    self.selfWriteChangeCounts.insert(changeCount)
                }
            }
        }
    ...
    func start() {
        ...
        Self.log("started at changeCount=\(lastChangeCount)")
        scheduleTimer()
    }
    ...
    private func pollPasteboard() {
        ...
        guard !consumeSelfWrite(changeCount: changeCount) else {
            Self.log("skipped self-write changeCount=\(changeCount)")
            return
        }
    ...
    private func capture(from pasteboard: NSPasteboard) {
        ...
        guard !shouldSkip(types: types) else {
            Self.log("skipped protected/transient pasteboard types=\(types.map(\.rawValue).joined(separator: ","))")
            return
        }
    ...
    deinit {
        timer?.invalidate()
        if let selfWriteObserver {
            NotificationCenter.default.removeObserver(selfWriteObserver)
        }
    }
}
...
extension Notification.Name {
    ...
    static let fifiPasteboardDidSelfWrite = Notification.Name("com.leafiy.fifi.pasteboard-did-self-write")
}
