...
struct PickerRowView: View {
    ...
    @ViewBuilder private var contextMenu: some View {
        Button("Send to Clipboard", action: onCopyToClipboard)
        Button("Copy as Plain Text") { onQuickAction(.copyPlainText) }
        switch item.type {
        case .color:
            Button("Copy as HEX") { onQuickAction(.copyColorHex) }
            Button("Copy as RGB") { onQuickAction(.copyColorRGB) }
            Button("Copy as HSL") { onQuickAction(.copyColorHSL) }
        case .url:
            Button("Open URL") { onQuickAction(.openURL) }
            Button("Copy Without Tracking") { onQuickAction(.copyCleanURL) }
        case .file:
            Button("Reveal in Finder") { onQuickAction(.revealInFinder) }
            Button("Quick Look") { onQuickAction(.quickLook) }
        default:
            EmptyView()
        }
        Divider()
