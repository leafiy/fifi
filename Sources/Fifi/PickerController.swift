...
final class PickerController {
    ...
    /// Applies the settings-driven transparency to the whole panel window.
    /// The frosted backdrop lives inside the SwiftUI content (see
    /// `PickerView`), so only the window alpha is handled here. Applying on
    /// show is enough — the outside-click monitor hides the panel whenever
    /// the settings window is used, so it is never visible while the
    /// setting changes.
    private func applyWindowOpacity() {
        panel.alphaValue = CGFloat(settingsStore.settings.currentWindowOpacity)
    }
